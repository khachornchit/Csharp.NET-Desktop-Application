...
{
    public partial class FormTransferTesting : Form
    {
        const int OPTION_COUNT_ALL = 1;
        const int OPTION_COUNT_GOOD_ONLY = 2;
        const int OPTION_COUNT_BAD_ONLY = 3;

        const string SERVER_TESTING_TABLE = Registration.SERVER_TESTING_TABLE;
        const string LOCAL_TESTING_TABLE = Registration.LOCAL_TESTING_TABLE;
        ...
        public FormTransferTesting(string sServerConnection, string sLocalConnection, string sWorkOrder)
        {
            InitializeComponent();
        ...
        int GetRowCount(int iOption)
        {
            int iRowCount = 0;
            sErrorMessage = null;
            sLocalQuery = "SELECT COUNT(*) FROM " + LOCAL_TESTING_TABLE + " WHERE WorkOrder='" + sWorkOrder + "'";
            switch (iOption)
            {
                case OPTION_COUNT_GOOD_ONLY:
                    sLocalQuery += " AND Result=1";
                    break;
        ...
        bool TransferToServer()
        {
            bool bSuccess = false;
            sErrorMessage = null;
            sLocalQuery = "SELECT TOP 1 * FROM " + LOCAL_TESTING_TABLE + " WHERE WorkOrder='" + sWorkOrder + "'";
            ...
            try
            {
                ...
                if (localOdbcDataReader.HasRows)
                {
                    ...
                    {
                        sServerQuery = "INSERT INTO " + SERVER_TESTING_TABLE + " ([WorkOrder],[BoxNumber],[Status],[F0_Unload],[F0_Load],[Q_Unload],[Q_Load],[ACT_Power],[ModulationDepth],[Temperature],[DeltaFreqVsTemp],[AID_HEX],[AID_DEC],[Result],[DateTime]) " +
                                       "VALUES('" +
                                       ...
                                       localOdbcDataReader[12] + "','" + localOdbcDataReader[13] + "','" + localOdbcDataReader[14] + "')";
                        sDateTime = localOdbcDataReader[14].ToString();
                        localOdbcDataReader.Close();
                        serverOdbcCommand.CommandText = sServerQuery;
                        iRetVal = serverOdbcCommand.ExecuteNonQuery();
                        if (iRetVal == 1)
                        {
                            sLocalQuery = "DELETE FROM " + LOCAL_TESTING_TABLE + " WHERE DateTime='" + sDateTime + "' AND WorkOrder='" + sWorkOrder + "'";
                            localOdbcCommand.CommandText = sLocalQuery;
